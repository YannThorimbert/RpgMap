...
##        if True:
            ...
            self.so_last = set()
            self.so_first = []
##            nframes = self.lm.nframes
            for o in so_log:
                if self.is_on_border(o):
                    self.border_objects.append(o)
        ...
        #################
        t = self.lm.t
        #blit objects on the core of the submaps. Within a given submap, all the
        #objects are correctly drawn, all at the same time.
    ...
    def is_on_screen(self, o, s):
        r,i = o.get_rect_and_img(s)
        return r.colliderect(self.me.screen_rect)

    def is_on_border(self, o):
        x,y = o.cell.coord
        gm = self.lm.current_gm
        smscx, smscy = gm.submap_size_cells
        modx = x%smscx
        if modx == 0 or modx == smscx-1:
            return True
        mody = y%smscy
        if mody == 0 or mody == smscy-1:
            return True
        return False
...
##                screen.blit(img, rect)
    ...
    def blit_intelligent(self, o, t, s):
        """This function should be called only if o is on the border of a
        submap."""
##        print(o.cell.coord, t, o.cell.get_chunk())
        ...
        for x in dx:
            ...
            for y in dy:
                yc = y0 + y
                coord = (xc, yc)
                if not(0 <= yc < self.lm.ny) or not(0 <= xc < self.lm.nx): #me.get_cell_at(coord, o.cell.get_chunk())
        ...
        #sorting order : (is_ground, bottom_coord, right_coord)
        neighbors.sort(key=lambda x:(not(x[1].is_ground),x[0][3],x[0][2])) #bottom, right
        gm = self.lm.current_gm
        finished = True
        for r,obj in neighbors: #include o itslef
            for x in dx:
                for y in dy:
                    coord = (x0+x, y0+y)
                    #done is False if submap of obj has not been built
                    done = gm.blit_object_at_frame_on_coord(obj, t, coord)
                    if not done:
                        finished = False
        if not has_unbuilt_neigh:
            o.blit_on_gm_border[t] = finished
##        o.blit_on_gm_border[t] = True
