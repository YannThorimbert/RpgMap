...
##        self.current_gm.draw_blits(screen, topleft, x0, y0, dx_pix, dy_pix, self.t)
    ...
    def draw_smart(self, screen, dx_pix, dy_pix, topleft, bottomright):
        ...
        y0 = self.current_y
##        print(topleft, x0,y0)
        self.current_gm.draw_smart(screen, x0, y0, dx_pix, dy_pix, self.t, topleft, bottomright)
##        self.current_gm.draw_blits(screen, topleft, x0, y0, dx_pix, dy_pix, self.t)
...
##        self.surfaces = [[[pygame.Surface(self.submap_size_pix) for frame in range(self.nframes)]
##                        for y in range(self.n_submaps[1])]
##                          for x in range(self.n_submaps[0])]
...
##                              for x in range(self.n_submaps[0])]
    ...
    def build_surface(self, x, y): #xxx peut etre eclaté en temps
        ...
        for t in range(self.nframes):
            ...
            if self.colorkey is not None:
                self.surfaces[surfx][surfy][t].set_colorkey(self.colorkey)


    def build_surface_smart(self, submap_x, submap_y): #xxx peut etre eclaté en temps et par cellule !!! ==> graphicalcell.built[t]
...
##        area_to_be_blitted = cell_here.clip(cell_object)
    ...
    def blit_object(self, obj):
        ...
        xpix = xobj*self.cell_size - surfx*self.submap_size_pix[0] + obj_rect.x
                ...
                if 0 <= cx < self.n_submaps[0] and 0 <= cy < self.n_submaps[1]:
                    ...
                    for t in range(self.nframes):
                        ...
                        if s:
                            ...
                            self.build_surface_smart(cx,cy)
...
##    Les autres cellules, donc, se redessinnent, mais on ne blitte que ca !
    ...
    def draw_smart(self, screen, x0, y0, dxpix, dypix, t, topleft, bottomright):
        #x0 = current_x from lm
        delta_x = dxpix + x0*self.cell_size
        delta_y = dypix + y0*self.cell_size
        xi, yi = topleft
