...
##            return self.graphical_maps[zoom][coord].imgs[self.t]
    ...
    def draw(self, screen, dx_pix, dy_pix):
        #dx_pix : number of pixels between the beginning of the cell and
        #   the actual pos.
        x0 = self.current_x
        y0 = self.current_y
        self.current_gm.draw(screen, x0, y0, dx_pix, dy_pix, self.t)
##        self.current_gm.draw_blits(screen, topleft, x0, y0, dx_pix, dy_pix, self.t)


    def draw_smart(self, screen, dx_pix, dy_pix, topleft, bottomright):
        #dx_pix : number of pixels between the beginning of the cell and
        #   the actual pos.
        x0 = self.current_x
        y0 = self.current_y
##        print(topleft, x0,y0)
        self.current_gm.draw_smart(screen, x0, y0, dx_pix, dy_pix, self.t)
##        self.current_gm.draw_blits(screen, topleft, x0, y0, dx_pix, dy_pix, self.t)
...
##            gm.build_surfaces_smart(self.colorkey)
...
##        cell_size = material_couples[0].get_cell_size(zoom_level)
    ...
    def generate_submaps_parameters(self, factor):
        nsubmap_x = factor//self.cell_size
        self.submap_size = (nsubmap_x*self.cell_size, )*2
    ...
    def build_surfaces(self, colorkey):
        ...
        #using pre-initialized surfaces saves about half a second of execution time!
...
##        self.surfaces = [[[pygame.Surface(self.submap_size) for frame in range(self.nframes)]
##                        for y in range(self.n_submaps[1])]
##                          for x in range(self.n_submaps[0])]

        b = time.time()
        #fill table of surfaces
        #opti : lot of things are constant in the loop below! xx
        for x,y in self: #unit : cell
            self.build_surface(x,y,colorkey)
        c = time.time()
        print("***",b-a,c-b)


    def build_surfaces_smart(self, colorkey):
        #create table of surfaces
##        pygame.Surface(self.submap_size)
    ...
    def build_surface(self, x, y, colorkey): #xxx peut etre eclaté en temps
        """x and y are in coords units from the whole graphical map"""
        #opti : lot of things are constant in the code below! xx
        surfx = x*self.cell_size//self.submap_size[0] #unit of submap_size : pixels
        ...
        for t in range(self.nframes):
            img = self[(x,y)].imgs[t] #see refresh_cell_types
            self.surfaces[surfx][surfy][t].blit(img, (xpix,ypix))
            if colorkey is not None:
                self.surfaces[surfx][surfy][t].set_colorkey(colorkey)

    def build_surface_smart(self, submap_x, submap_y, colorkey): #xxx peut etre eclaté en temps
        """x and y are the submap indices"""

        #opti : lot of things are constant in the code below! xx
##        submap_x = x*self.cell_size//self.submap_size[0] #unit of submap_size : pixels

        cells_per_submap_x = self.submap_size[0] // self.cell_size
        x0 = submap_x * cells_per_submap_x #total x cell of the whole graphical map
        xpix = x0*self.cell_size - submap_x*self.submap_size[0]

        cells_per_submap_y = self.submap_size[1] // self.cell_size
        y0 = submap_y * cells_per_submap_y #total x cell of the whole graphical map
        ypix = y0*self.cell_size - submap_y*self.submap_size[1]

        for x in range(self.submap_size[0]):
            for y in range(self.submap_size[1]):
                for t in range(self.nframes):
                    img = self[(x0+x,y0+y)].imgs[t] #xx d'ou vient ce truc ???
                    s = self.surfaces[submap_x][submap_y][t]
                    if not s:
                        s = pygame.Surface(self.submap_size)
                    s.blit(img, (xpix+x*self.cell_size,ypix+y*self.cell_size))
                    if colorkey is not None:
...
##    Les autres cellules, donc, se redessinnent, mais on ne blitte que ca !
    ...
    def draw(self, screen, x0, y0, dxpix, dypix, t):
        ...
        for x in range(self.n_submaps[0]):
            ...
            for y in range(self.n_submaps[1]):
                ...
                #draw submap border:
                pygame.draw.rect(screen, (0,0,255), pygame.Rect(posx,posy,self.submap_size[0],self.submap_size[1]),1)

    def draw_smart(self, screen, x0, y0, dxpix, dypix, t):
        #x0 = current_x from lm
        delta_x = dxpix + x0*self.cell_size
        delta_y = dypix + y0*self.cell_size
        for x in range(self.n_submaps[0]):
            ...
            for y in range(self.n_submaps[1]):
                ...
                if surf:
                    ...
                    self.build_surface_smart(x,y,self.colorkey)
