...
##    def build_objects_list(self):
##        s = self.lm.get_current_cell_size()
####        import bisect
##        self.objects_tuples = []
##        for nm in self.neigh_maps.values():
##            for o in nm.lm.static_objects:
##                r,i = o.get_rect_and_img(s)
##                ntuple = (not(o.is_ground), r.bottom, r.right, r, o)
##                self.objects_tuples.append(ntuple)
####                bisect.insort(self.objects_tuples, ntuple)
##        self.objects_tuples.sort(key=lambda x:x[0:4])
##        print("Objects rebuilt")

##    def build_objects_to_display(self):
##        s = self.lm.get_current_cell_size()
##        import bisect
##        self.objects_tuples = []
##        for nm in self.neigh_maps.values():
##            for o in nm.lm.static_objects:
##                r,i = o.get_rect_and_img(s)
##                if r.colliderect(self.screen_rect):
##                    ntuple = (not(o.is_ground), r.bottom, r.right, r, o)
####                    self.objects_tuples.append(ntuple)
##                    bisect.insort(self.objects_tuples, ntuple) #xx
##        self.objects_tuples.sort(key=lambda x:x[0:4])
...
##                    self.objects_tuples.append(ntuple)
    ...
    def draw_objects(self):
        if self.delta_cam or self.game.t==0:
            self.build_objects_to_display()
        for ntuple in self.objects_tuples:
            r = ntuple[3]
            img = ntuple[-1].get_current_img()
            self.screen.blit(img, r)
        #no impact on performance:
##        blits = [(t[-1].get_current_img(),t[3]) for t in self.objects_tuples]
##        self.screen.blits(blits)

##        pygame.image.save(self.screen, "shot"+str(self.game.t).zfill(8)+".png")
...
##        pygame.draw.rect(self.screen, (255,255,255), self.cam.rmouse, 1)
    ...
    def draw_no_update(self):
        ...
        self.cam.draw_map(self.screen, self.show_grid_lines)
        #blit objects
        self.cam.draw_objects(self.screen, self.objects_tuples, True)
        #blit smoke effects
        self.game.refresh_smokes()
        #blit right pane and draw rect on minimap
        self.e_box.blit()
